...
# ============================================================================
# API Functions for Flask server
# ============================================================================

def load_model(model_dir):
    """
    Carga el modelo Voxtral y devuelve un objeto modelo
    Retorna: diccionario con el modelo cargado
    """
    sf_path = os.path.join(model_dir, "consolidated.safetensors")
    if not os.path.exists(sf_path):
        raise FileNotFoundError(f"Modelo no encontrado en {sf_path}")

    print(f"Cargando modelo desde {sf_path}", file=sys.stderr)
    sf_file = safe_open(sf_path, framework="pt")

    print("Cargando decoder...", file=sys.stderr)
    decoder = Decoder(sf_file)

    # Precargar mel filters
    mel_filters = torch.tensor(compute_mel_filters(), dtype=torch.float32)
...
def transcribe_file(model, wav_path):
    ...
    Args:
        model: diccionario devuelto por load_model()
        wav_path: ruta al archivo WAV
    Returns:
        texto transcrito
    """
    sf_file = model['sf_file']
    ...
    # Streaming-format prefix + offline audio padding
    prompt_ids = [TOKEN_BOS] + [TOKEN_STREAMING_PAD] * (N_LEFT_PAD_TOKENS + N_DELAY_TOKENS)
    padded = pad_audio_streaming(audio_array).astype(np.float32)
    ...
    # Decoder prefill and generation
    n_audio = adapter_out.shape[0]
    L = len(prompt_ids)

    prompt_ids_t = torch.tensor(prompt_ids, dtype=torch.long)
    prefix_text_embeds = decoder.embed_tokens(prompt_ids_t)
    prefix_embeds = adapter_out[:L] + prefix_text_embeds
    ...
    # Decode
    text = tokenizer(generated).strip()
